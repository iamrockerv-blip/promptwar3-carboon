...
export default function ProfileSettings() {
  ...
  const [activeCategory, setActiveCategory] = useState<
    'transport' | 'diet' | 'energy' | 'travel' | 'consumption'
  >('transport');
    ...
    <div className="w-full space-y-6">
      ...
      <div className="p-6 md:p-8 rounded-3xl bg-neutral-900/40 border border-white/5 backdrop-blur-xl space-y-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-neutral-400 mt-0.5">
                Refine your baseline answers to instantly recalculate your Carbon Twin&apos;s
                profile.
              </p>
            </div>
          </div>
        ...
        <div className="p-3.5 rounded-2xl bg-white/5 border border-white/5 flex items-start gap-2.5 text-xs text-neutral-400 leading-normal">
          ...
          <p>
            Adjusting these answers updates your{' '}
            <span className="font-semibold text-white">baseline lifestyle</span>. If you want to
            simulate temporary habit shifts (like a sandbox test), use the{' '}
            <span className="font-semibold text-white">Habit Sandbox</span> tab instead.
          </p>
        </div>
      </div>
      ...
      {question && (
        ...
        >
          <div>
            ...
            <h4 className="text-base font-bold text-white mt-1 font-display">{question.text}</h4>
          </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <span
                      className={`font-bold text-xs ${isSelected ? 'text-white font-extrabold' : 'text-neutral-300'}`}
                    >
                      {option.label}
                    </span>
      ...
      <div className="p-6 rounded-3xl bg-red-950/10 border border-red-500/10 backdrop-blur-xl space-y-4">
        <div>
          <h4 className="text-sm font-bold text-red-400 font-display">Danger Zone</h4>
          <p className="text-xs text-neutral-400 mt-1">
            Purging your twin data will delete all saved habits, twin scores, and local coach
        ...
        <button
          ...
          onClick={() => {
            if (
              confirm(
                'Are you sure you want to purge all local twin data? This will reset your progress.'
              )
            ) {
