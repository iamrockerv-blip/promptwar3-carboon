...
[![Quality Gate](https://github.com/iamrockerv-blip/promptwar3-carboon/actions/workflows/ci.yml/badge.svg)](https://github.com/iamrockerv-blip/promptwar3-carboon/actions/workflows/ci.yml)
[![React Doctor](https://github.com/iamrockerv-blip/promptwar3-carboon/actions/workflows/react-doctor.yml/badge.svg)](https://github.com/iamrockerv-blip/promptwar3-carboon/actions/workflows/react-doctor.yml)
...
1. **Clone the repository**:
   ...
   ```bash
   git clone https://github.com/iamrockerv-blip/promptwar3-carboon.git
   cd promptwar3-carboon
   ```
...
- **Complete local quality gate**: `npm run check`
...
- **Coverage and accessibility tests**: `npm run test:coverage`
...
Coverage thresholds and every command above are enforced in
[GitHub Actions](.github/workflows/ci.yml), not merely documented.
...
3. In the **Environment Variables** configuration, add:
   - `GEMINI_API_KEY`: _(your Gemini key from Google AI Studio)_
   - `NEXT_PUBLIC_APP_URL`: _(your Vercel deployment URL)_
...
The included multi-stage image runs the Next.js standalone server as a non-root user:

```bash
docker build -t carbon-twin-ai .
docker run --rm -p 3000:3000 -e GEMINI_API_KEY=your_key carbon-twin-ai
```

Container platforms can probe `GET /api/health`.
...
| Axis                  | Evidence                                                                                                                                                                     |
| --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Code Quality**      | Strict TypeScript, layered architecture, Prettier, ESLint, React Doctor, contribution and architecture guides                                                                |
| **Security**          | No committed secrets, strict bounded Zod schemas, body-size limits, sanitized errors, rate limiting, CSP/security headers, zero-vulnerability audit gate, non-root container |
| **Efficiency**        | Next.js static rendering where possible, dynamic loading for heavy charts/dashboard panels, standalone production output, deterministic local calculations                   |
| **Testing**           | Unit, integration, API security, exhaustive 1,920-combination math validation, axe accessibility tests, and enforced coverage gates                                          |
| **Accessibility**     | Semantic controls, keyboard quiz, skip link, visible focus, reduced motion, live regions, busy states, chart table equivalents, JSX accessibility lint                       |
| **Problem Alignment** | Understand footprint → simulate consequences → reduce through quantified shifts, quests, personalized recommendations, and AI coaching                                       |

See [Architecture](docs/ARCHITECTURE.md), [Contributing](CONTRIBUTING.md), and
[Changelog](CHANGELOG.md) for implementation evidence.
