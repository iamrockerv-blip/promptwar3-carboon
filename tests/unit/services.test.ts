...
describe('API service clients', () => {
  ...
  it('posts the carbon twin payload and returns parsed data', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith('/api/generate-twin', expect.objectContaining({
      method: 'POST',
      headers: { 'Content-Type': 'application/json' }
    }));
  });
  ...
  it('posts coach context and rejects non-success responses', async () => {
    vi.spyOn(globalThis, 'fetch')
      .mockResolvedValueOnce(new Response(JSON.stringify({ text: 'Try public transit.' }), {
        status: 200,
        headers: { 'Content-Type': 'application/json' }
      }))
    ...
    await expect(sendCoachMessageApi('Help', [], 7.3, 'sapphire', 'Story'))
      .resolves.toEqual({ text: 'Try public transit.' });
