...
1. **Report Privately**: Open a Private Vulnerability Report through the repository's
   **Security** tab, or contact the repository owner (`iamrockerv-blip`) through their
   GitHub profile.
...
### 5. Request and Error Boundaries

- **Bounded Bodies**: JSON requests are restricted by content type and byte size.
- **Safe Failures**: Unexpected server and provider errors return generic client messages;
  internal exception details are not exposed.
- **No-Store API Responses**: Personalized AI responses are marked `Cache-Control: no-store`.

### 6. Supply Chain and Deployment

- **Automated Audit**: CI fails on moderate-or-higher known dependency vulnerabilities.
- **Reproducible Install**: CI and Docker use `npm ci` with the committed lockfile.
- **Least Privilege Runtime**: The production container runs as an unprivileged user.
